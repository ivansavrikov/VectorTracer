...
@app.post("/tracer/")
async def vectorize(file: UploadFile = File(...), colors: str = Form(...), detailing: int = Form(...)):
	...
	svg_paths = UTracer.trace(image, detailing)
...
# 	svg_image = UTracer.put_image(image)
# 	svg_code = SVG.svg_open(image.width, image.height) + SVG.metadata() + svg_image + SVG.svg_close()
# 	svg_data = BytesIO()
# 	svg_data.write(svg_code.encode("utf-8"))
